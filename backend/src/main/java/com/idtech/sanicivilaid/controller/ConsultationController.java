...
import com.idtech.sanicivilaid.entity.Consultation;
...
public class ConsultationController {
    ...
    public Result<ConsultationVO> submit(@RequestBody Consultation consultation) {
        ...
        if (StrUtil.isNotBlank(questionCn)) {
            ...
            // 中文提问为空，使用彝文进行模糊搜索
            faqList = bizCivilFaqRepository.searchBySaniKeyword(questionSani);
        }
        ...
        if (CollUtil.isNotEmpty(faqList)) {
            ...
            consultation.setSaniFont("Yi Script");
            consultationRepository.save(consultation);
        ...
        // 数据库中没有相关记录，调用 AI
        String searchKeyword = StrUtil.isNotBlank(questionCn) ? questionCn : questionSani;
        AiCivilArticleVO aiResult = callAi(searchKeyword);
        if (aiResult != null) {
            consultation.setAnswerCn(aiResult.getContentCn());
            consultation.setAnswerSani(aiResult.getContentSani());
            consultation.setStatus(1);
            consultation.setSaniFont("Microsoft Yi Baiti");
            consultationRepository.save(consultation);
    ...
    private AiCivilArticleVO callAi(String keyword) {
        ...
        String prompt = String.format(
                "你是一个专业的中国法律助理。用户查询了民法典相关内容：'%s'。我们的数据库暂未收录。 " +
                        ...
                        "必须包含以下两部分：1. 汉文解释。2. 对应的规范彝文（Sani Yi）翻译。 " +
                        "请以 JSON 格式返回，字段名为 contentCn 和 contentSani。", keyword);
    ...
    private ConsultationVO toVO(Consultation consultation) {
        return ConsultationVO.builder()
                ...
                .questionSani(consultation.getQuestionSani())
                .answerCn(consultation.getAnswerCn())
                .answerSani(consultation.getAnswerSani())
                .status(consultation.getStatus())
                .createTime(consultation.getCreateTime())
                .saniFont(consultation.getSaniFont())
                .build();
    }
}
